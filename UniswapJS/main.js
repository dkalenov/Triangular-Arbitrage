...
function calculateArbitrage(amountIn, amountOut, surfaceObj) {
  ...
  let threshold = 0; // try to set 1.5
...
async function getPrice(factory, amtIn, tradeDirection) {
  // Get Provider
  const provider = new ethers.providers.JsonRpcProvider("https://mainnet.infura.io/v3/ff30285590e346c08dc4b93e6c9ee23d");
  const ABI = [
  ...
  let amountInParsed = ethers.utils.parseUnits(amtIn, inputDecimalsA).toString();
...
getDepth(amountIn=100); // amount of the first coin
